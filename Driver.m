...
Create_Struct;
for subj =1:10
    load(load(['subject_0' num2str(i) '.mat'];
    ...
    for i=1:5
        %Extracting the data
        spectDataopen=SIGNAL(eye_opened(i):(eye_opened(i)+6*Fs-1),channel);
        spectDataclose=SIGNAL(eye_closed(i):(eye_closed(i)+6*Fs-1),channel);
...
AlphaPower.AllSubjAvg(subj,1)=(open_avg/5);
AlphaPower.AllSubjAvg(subj,2)=(close_avg/5);
end
% 
% %% Subject 2
% 
